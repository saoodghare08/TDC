...
export default function Stack({
    ...
    // Initialize random rotations only on the client to avoid hydration mismatch
    useEffect(() => {
        if (randomRotation && cards.length > 0) {
            const rotations = {};
            cards.forEach((_, index) => {
                rotations[index + 1] = Math.random() * 10 - 5;
            });
    ...
    return (
        ...
        >
            {stack.map((card, index) => {
                const randomRotate = cardRotations[card.id] || 0;
                return (
                    <CardRotate
                        key={card.id}
